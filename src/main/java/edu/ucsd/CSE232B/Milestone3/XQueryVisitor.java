...
import edu.ucsd.CSE232B.Milestone2.XMLDataParserXquery;
...
public class XQueryVisitor extends XQueryGrammarBaseVisitor<LinkedList<Node>> {
    ...
    /*
    This function performs join operation between 2 blocks of tuples returned either from for statements or
    internal joins.

    At first, it visits the 2 arguments (Xqueries) for the join to evaluate the list of nodes(tuples) for each argument.
    Then, every node(tuple) in the right argument is hashed according to the String value of each join attribute, i.e.
    each node would be stored in a HashMap where the key would be the string representation of evaluation of join
    attribute and the value would be a list of nodes (tuples) matching the string representation.

    Finally, hash every node (tuple) of the left argument using its join attributes, and for matching key
    in the HashMap, join the node with the list of nodes in the right argument (i.e. value for the matching key)
    as these are the nodes(tuples) satisfying the join condition.
     */
    @Override
    public LinkedList<Node> visitXQueryJoin(XQueryGrammarParser.XQueryJoinContext ctx) {

        //This list will hold the resulting nodes
        LinkedList<Node> resultNodes = new LinkedList<>();

        //Create a temporary list from current list of nodes
        LinkedList<Node> tmp = this.nodes;

        //Fetch nodes from result of evaluation of xq1 (left argument of join)
        LinkedList<Node> leftNodes = visit(ctx.xq(0));

        //Restore the current list of nodes
        this.nodes = tmp;

        //Fetch nodes from result of evaluation of xq2 (right argument of join)
        LinkedList<Node> rightNodes = visit(ctx.xq(1));

        //Fetch the list of join attributes from the parse tree
        List<TerminalNode> leftAttributes = ctx.joinAttributes(0).IDENTIFIER();
        List<TerminalNode> rightAttributes = ctx.joinAttributes(1).IDENTIFIER();

        //Raise an error if sizes of the list of join attributes for both the arguments are not equal
        if(leftAttributes.size() != rightAttributes.size()){
            xmlDataParser.printError("Sizes of Join Attributes list are not equal!");
        }

        //Create a HashMap which will store the (key,value) mapping for the join attribute of right argument
        //This HashMap will act as a Hash based Index on the right argument
        HashMap<String, LinkedList<Node>> rightIndex = new HashMap<>();

        //For every node in the right argument, compute its hash value as per the join attributes
        for(int i=0; i<rightNodes.size(); i++){
            Node rightNode = rightNodes.get(i);

            //Fetch the key (string representation) for the tuple based on its list of join attributes
            String keyValue = xmlDataParser.keyToNodeAttrMapping(rightNode,rightAttributes);

            //Create a new (key,value) pair if key does not exist
            rightIndex.putIfAbsent(keyValue, new LinkedList<>());

            //Add the node (tuple) to the list of nodes represented by the computed hash value (String representation)
            rightIndex.get(keyValue).add(rightNode);
        }

        //For every node(tuple) in the left argument, we perform join for matching key (string representation)
        for(int i=0; i<leftNodes.size(); i++){
            Node leftNode = leftNodes.get(i);

            //Fetch the key (string representation) for the tuple based on its list of join attributes
            String keyValue = xmlDataParser.keyToNodeAttrMapping(leftNode,leftAttributes);

            //If there is matching key, we perform a join as it means the join condition is satisfied
            if(rightIndex.containsKey(keyValue)){

                //Fetch nodes(tuples) to be joined with the node of left argument
                LinkedList<Node> rightJoiningTuples = rightIndex.get(keyValue);

                //For every node (tuple) in the list of nodes which satisfies the join condition
                for(int j=0; j<rightJoiningTuples.size();j++){
                    Node joiningNode = rightJoiningTuples.get(j);

                    //Create a joined tuple with attributes of both the arguments in the join
                    LinkedList<Node> joinedTuple = new LinkedList<>();

                    //Add all children(attributes) of the left argument
                    joinedTuple.addAll(xmlDataParser.getChildren(leftNode));

                    //Add all children(attributes) of the right argument
                    joinedTuple.addAll((xmlDataParser.getChildren(joiningNode)));

                    //Add the resulting joined tuple to the list of resulting nodes by creating an element <tuple>
                    resultNodes.add(xmlDataParser.makeElement(leftNode.getNodeName(),joinedTuple));
                }
            }

        }
        //Set current list of nodes as resulting nodes
    ...
    /* This function returns null as utilising the result of visiting join attributes is invalid */
    @Override
    public LinkedList<Node> visitJoinAttributes(XQueryGrammarParser.JoinAttributesContext ctx) {
        return null;
    }
